...
---
trigger: always_on
description: Enterprise SPFx development standards and Fluent UI 2 Griffel rules
---
...
## SPFx Architecture & Property Pane
- Configuration properties flow strictly from `@microsoft/sp-property-pane` (`this.properties`) down into root React components via typed interfaces (`I<WebPartName>Props`). React components must never directly mutate `this.properties`.
...
- Maintain `feature.JSON` (matrix) and `ProjectStructure.JSON`. Use `replace_file_content` to preserve existing keys.
